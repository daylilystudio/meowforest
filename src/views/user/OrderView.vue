  ...
  <ShopLayout :process="2" breadcrumb="Order Finish" nextBtn="Back To Home" @onGoNext="router.push('/')">
    <template v-slot:content>
      <div v-if="orderInfo!==null">
        <img src="@/assets/img/thx.png" class="tw:block tw:w-3/5 tw:sm:w-60 tw:h-auto tw:mx-auto" alt="thank you!">
        <h4 class="tw:bg-gray-200 tw:rounded-lg tw:py-1 tw:px-4">Order Details
          <span class="tw:text-notice tw:border-notice tw:border tw:border-solid tw:text-sm tw:rounded-full tw:px-2 tw:ml-1">{{ orderInfo.is_paid ? 'Paid' : 'Unpaid' }}</span>
        </h4>
        <p class="tw:text-right tw:font-bold tw:text-primary tw:my-2 tw:mr-4">Order No. {{ orderInfo.create_at }}
          <br class="tw:block tw:md:hidden">
        ...
        <section class="tw:md:grid tw:grid-cols-2 tw:px-4 tw:leading-7">
          ...
          <div v-if="orderInfo.user.payment_method==='atm'"
            class="tw:text-primary tw:border-primary tw:border tw:border-solid tw:shadow-inner tw:rounded-xl tw:font-bold tw:col-span-2 tw:md:flex tw:items-center tw:py-3 tw:mt-2">
            <p class="tw:text-lg tw:text-center tw:md:px-8"><FontAwesomeIcon :icon="['fas', 'cat']" /> ATM transfer Info</p>
            <p class="tw:grid tw:gap-1 tw:text-center tw:md:text-left tw:my-4 tw:md:my-0">
              <span>Bank Code : 333 (Meow Bank)</span>
              <span>Account : 00001234567890</span>
              <span class="tw:text-notice">Deadline : {{ filter.date(orderInfo.create_at*1000, 3, false) + ' 23:00' }}</span>
            </p>
          </div>
        </section>
        <h4 class="tw:bg-gray-200 tw:rounded-lg tw:py-1 tw:px-4 tw:mt-10 tw:mb-2">Customer Details</h4>
        <section class="tw:md:grid tw:grid-cols-2 tw:leading-7 tw:px-4">
        ...
        <section :class="(openProducts?'tw:h-auto ':'tw:h-0')" class="tw:overflow-hidden tw:px-4">
          <div v-for="(item, i) in orderInfo.products" :key="`order${i}`" class="tw:grid tw:grid-cols-12 tw:gap-y-4 tw:items-center tw:py-4 tw:border-b tw:border-solid tw:border-gray-200">
            <span class="tw:col-span-12 tw:md:col-span-6">
              {{ item.product.title }}
            </span>
            ...
            <span class="tw:col-span-4 tw:md:col-span-2">{{ item.qty }}</span>
            <span class="tw:col-span-4 tw:md:col-span-2 tw:flex tw:justify-between">
          ...
          <p class="tw:mt-5 tw:text-right">
            ...
            <span>Total / </span><span class="tw:font-bold tw:text-primary tw:ml-1">{{ '$'+(Math.ceil(orderInfo.total)+orderInfo.user.shipping_money) }}</span>
          </p>
        </section>
      </div>
