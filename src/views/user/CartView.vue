    ...
    <template v-slot:content>
      <NSpin :show="globalStore.loading" class="tw:px-2">
        <template v-if="globalStore.cart.carts?.length>0">
          <div class="tw:hidden tw:md:grid tw:bg-gray-200 tw:rounded-lg tw:py-2.5 tw:px-3 tw:grid-cols-12">
          ...
          <div class="tw:block tw:md:hidden tw:bg-gray-200 tw:rounded-lg tw:py-1 tw:px-4">Shop List</div>
            <div v-for="(item, i) in globalStore.cart?.carts" :key="`cart${i}`" class="tw:relative tw:grid tw:grid-cols-12 tw:gap-y-4 tw:items-center tw:py-5 tw:border-b tw:border-solid tw:border-gray-200">
              <a href=”#” @click.prevent="router.push('/products/'+item.product.id)" class="tw:col-span-12 tw:md:col-span-6 tw:flex tw:items-center tw:cursor-pointer">
                <img :src="item.product.imagesUrl[0]" :alt="item.product.title" class="tw:w-28 tw:mr-2">
                {{ item.product.title }}
              </a>
              <span class="tw:col-span-3 tw:md:col-span-2 tw:flex tw:justify-between">
                <span class="tw:text-primary tw:font-bold">${{ item.product.price }}</span>
                <span class="tw:mr-4 tw:block tw:md:hidden">X</span>
              </span>
              <NInputNumber @update:value="updateCart($event, item.id, item.product.id)" class="tw:w-4/5 tw:col-span-5 tw:md:col-span-2 tw:mt-1 tw:text-center" v-model:value="item.qty" :min="1" size="small" button-placement="both" />
              <span class="tw:col-span-4 tw:md:col-span-2 tw:flex tw:justify-between">
                <span class="tw:block tw:md:hidden">=</span>
                <span class="tw:text-primary tw:font-bold">${{ item.total }}</span>
              </span>
              <p @click="delCart(item.id)" class="tw:absolute tw:right-0 tw:top-8 tw:md:top-1/2 tw:-translate-y-1/2 tw:p-2 tw:cursor-pointer">
        ...
        <div v-else class="tw:opacity-50 tw:w-2/3 tw:md:w-72 tw:mx-auto tw:mt-16 tw:mb-6">
          <img src="@/assets/img/noData.png" class="tw:w-full tw:h-auto" alt="no data">
        </div>
      </NSpin>
    </template>
    <template v-slot:order>
      <section v-if="globalStore.cart.carts?.length>0" class="tw:bg-third tw:rounded-2xl tw:flex tw:flex-col tw:md:flex-row tw:gap-6 tw:md:gap-14 tw:p-6">
...
<style lang="scss" scoped>
  input ~ i {
    border: 2px solid var(--tw-color-theme);
    opacity: .4;
    position: relative;
  }
  input:checked ~ i {
