    ...
    <template v-if="globalStore.cart.carts?.length>0 && !route.path.match(/order/gi)">
      ...
      <div class="tw-text-xl tw-text-right tw-font-bold">
        <p class="tw-mb-2">Your Basket
          <span class="tw-ml-4 text-primary">NTD. {{globalStore.cart.total}}</span>
        </p>
        <p v-if="globalStore.cart.carts[0]?.coupon" class="tw-mb-2">- Discount
          <span class="tw-ml-4 text-primary">{{globalStore.cart.total-Math.ceil(globalStore.cart.final_total)}}</span>
        </p>
        <p class="tw-pb-4 tw-border-b tw-border-solid border-theme">+ Shipping
          <span class="tw-ml-4 text-primary">{{globalStore.shippingMoney}}</span>
        </p>
        <p class="tw-mt-4">Order Total
          <span class="tw-ml-4 text-second">NTD. {{Math.ceil(globalStore.cart.final_total)+(globalStore.shippingMoney)}}</span>
        </p>
      </div>
    </template>
