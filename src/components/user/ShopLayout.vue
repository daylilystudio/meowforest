<template>
  ...
  <main class="container">
    ...
    <section class="tw:shadow-main tw:bg-white tw:rounded-2xl tw:mb-8 tw:p-4 tw:sm:p-6">
      <TheProcessBar :process="process" />
      <slot name="content" />
    </section>
    <template v-if="globalStore.cart.carts?.length>0 && !route.path.match(/order/gi)">
      <slot name="order" />
      <h5 class="tw:mt-8 tw:mb-3 tw:sm:mb-0">ORDER SUMMARY</h5>
    ...
    <a href=”#” v-if="nextBtn" @click.prevent="emits('onGoNext', '')" class="tw:hover:brightness-90 tw:bg-second tw:text-theme tw:rounded-full tw:block tw:font-bold tw:text-center tw:w-3/5 tw:sm:w-60 tw:p-4 tw:mt-10 tw:mx-auto" :class="nextBtnAllow?'tw:cursor-pointer':'tw:cursor-not-allowed'">
