<template>
  <nav class="nav-container tw:h-14 tw:xl:h-16 tw:fixed tw:top-3 tw:sm:top-6 tw:left-1/2 tw:-translate-x-1/2
  tw:rounded-full tw:shadow-xl tw:bg-white tw:flex tw:items-center tw:p-1.5 tw:z-20">
    ...
    <div :class="{'active':openMobileNav}" class="menuList tw:fixed tw:top-14 tw:left-6 tw:right-6 tw:md:static tw:flex tw:flex-col tw:md:flex-row tw:gap-y-7 tw:gap-x-6 tw:lg:gap-x-10 tw:items-center tw:justify-center
    tw:bg-white tw:text-primary tw:font-bold tw:lg:text-xl tw:md:ml-auto tw:rounded-b-2xl tw:shadow-xl tw:md:shadow-none">
      <RouterLink v-for="list in globalStore.menu" :key="list.link" @click="openMobileNav=false" :to="list.link" class="menuA">
        {{ list.name }}
      </RouterLink>
    </div>
    <a href=”#” @click.prevent="openMobileNav=!openMobileNav" :class="{'active':openMobileNav}" class="menuBtn tw:ml-auto tw:flex tw:md:hidden tw:cursor-pointer">
      <span v-for="i in 3" :key="'menu'+i" class="tw:bg-theme" />
    </a>
    <RouterLink to="/cart" :class="{'tw:cursor-not-allowed':route.path==='/cart', 'shake':globalStore.addingCart}" title="Go Shopping Cart" class="tw:bg-primary tw:relative tw:rounded-full tw:p-2.5 tw:md:p-3 tw:ml-6 tw:cursor-pointer tw:hover:brightness-90">
  ...
  <div @click="openMobileNav=false" :class="(!openMobileNav?'tw:pointer-events-none tw:opacity-0':'tw:opacity-50')" class="tw:bg-theme tw:block tw:md:hidden tw:fixed tw:w-full tw:h-full tw:top-0 tw:z-10" />
