<template>
  <div class="tw:fixed tw:bottom-3 tw:sm:bottom-6 tw:right-3 tw:md:right-5 tw:z-20">
    <a href=”#” v-for="item in list" :key="item.icon"
      ...
      tw:rounded-full tw:mb-2 tw:cursor-pointer">
  ...
  <NModal v-model:show="showModal" :block-scroll="false" title="My Fav Product" preset="card" style="max-width:95%; width: 650px">
    <template v-for="item in globalStore.products" :key="item.id">
      <div v-if="globalStore.isFav['meowForestFav' + item.id]" class="tw:flex tw:justify-between tw:items-center tw:py-2">
        ...
        <a href=”#” @click.prevent="showModal=false;router.push('/products/'+item.id)" class="tw:flex-1 tw:flex tw:items-center tw:cursor-pointer">
          <img :src="item.imagesUrl[0]" :alt="item.title" class="tw:w-16 tw:sm:w-24 tw:rounded-lg">
          <span class="tw:text-theme tw:md:text-base tw:pl-4">{{ item.title }} / ${{ item.price }}</span>
        </a>
        <a href=”#” @click.prevent="globalStore.addCart(item.id, 1);clickItem=item.id" title="Add To Cart" class="tw:bg-primary tw:w-10 tw:h-10 tw:flex tw:items-center tw:justify-center tw:rounded-full tw:hover:brightness-90 tw:cursor-pointer">
    ...
    <div v-if="Object.values(globalStore.isFav).every(el => el === false)" class="tw:text-center">
      <img src="@/assets/img/noData.png" class="tw:w-4/5 tw:sm:w-48 tw:h-auto" alt="no data">
    </div>
  </NModal>
</template>
